 ...
 * - For any occurrence of /\sfrom\s+"(\..*)"/ the path will be extracted and resolved from the currently parsed file, and then resolved to a relative path from the root of the git repo, then the whole `from` clause will be replaced with `from "https://deno.land/x/${projectName}/${relativePathFromGitRoot}"`.
...
function processLineForImport(
  ...
  return (line: string): string => {
    ...
    if (match) {
      ...
      return line.replace(
        /\sfrom\s+"(\..*)"/,
        ` from "https://deno.land/x/truenas_unlock_dataset_once_after_each_boot/${step2}"`,
      );
    }
    return line;
  };
